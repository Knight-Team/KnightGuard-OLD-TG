...
local api_key = "263175540:AAGel0R2cn4LCsWZqpLtZZbJl0s70-BBfvU"
http.TIMEOUT = 10
local BASE_URL = "https://api.pwrtelegram.xyz/bot"..api_key
...
function get_cli_receiver(msg)
  ...
  if msg.chat.type == "group" then
    ...
    local chat = id:gsub("-","")
    return tonumber(chat)
  end
  if msg.chat.type == "supergroup" then
    local id = tostring(msg.chat.id)
    local chat = id:gsub("-100","")
    return tonumber(chat)
  end
end
...
function is_vip_group(msg)
    local var = false
    if redis:sismember("VIP:GROUPS",msg.to.id) then
        var = true
    end
    return var
end
...
function is_owner(msg)
  ...
  local hash = "owner:"..msg.to.id..":"..msg.from.id
...
function is_owner2(user_id, group_id)
  ...
  local hash = "owner:"..chat_id..":"..group_id
  if redis:get(hash) then
    var = true
  end
...
function is_momod_api(from,to)
  local var = false
  local data = load_data(_config.moderation.data)
  local user = from
  if data[tostring(to)] then
    if data[tostring(to)]['moderators'] then
      if data[tostring(to)]['moderators'][tostring(user)] then
        var = true
      end
    end
  end
  ...
  if data[tostring(to)] then
    if data[tostring(to)]['set_owner'] then
      if data[tostring(to)]['set_owner'] == tostring(user) then
        var = true
      end
    end
  end
...
function send_api_msg(receiver, text, disable_web_page_preview, markdown, reply_to_message_id)
  ...
  if tonumber(reply_to_message_id) > 0 then
