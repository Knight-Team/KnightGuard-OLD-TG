...
local function kick_ban_res(extra, success, result)
	  ...
	  if success == 0 then
		return send_large_msg(receiver, "\n<b>Cannot Find User By That UserName! </b>")
	  end
      ...
      elseif get_cmd == 'ban' then
        ...
        send_large_msg(receiver, '\n<b>»User </b><code>@'..member..' </code><b>Banned!« </b>\n<b>->Info User:</b>\n<b>》ID: </b><code>'..member_id..' </code>\n<b>》User Name: </b>@'..member..'\n<b>》Link User: </b>\nhttps://telegram.me/'..member..'\n<b>»Status: User Banned!« </b>\n')
...
local bannedhash = 'banned:'..msg.from.id..':'..msg.to.id
      ...
      elseif get_cmd == 'unban' then
        send_large_msg(receiver, '\n<b>»User </b><code>@'..member..' </code><b>UnBanned!« </b>\n<b>->Info User:</b>\n<b>》ID: </b><code>'..member_id..' </code>\n<b>》User Name: </b>@'..member..'\n<b>》Link User: </b>\nhttps://telegram.me/'..member..'\n<b>»Status: User UnBanned!« </b>\n')
        local hash =  'banned:'..chat_id
        redis:srem(hash, member_id)
        return 'User '..user_id..' unbanned'
      elseif get_cmd == 'banall' then
        send_large_msg(receiver, '\n<b>»User </b><code>@'..member..' </code><b>Globally Banned« </b>\n<b>->Info User:</b>\n<b>》ID: </b><code>'..member_id..' </code>\n<b>》User Name: </b>@'..member..'\n<b>》Link User: </b>\nhttps://telegram.me/'..member..'\n<b>»Status: User Globally Banned!« </b>\n')
		banall_user(member_id)
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, '\n<b>»User </b><code>@'..member..' </code><b>Globally UnBanned« </b>\n<b>->Info User:</b>\n<b>》ID: </b><code>'..member_id..' </code>\n<b>》User Name: </b>@'..member..'\n<b>》Link User: </b>\nhttps://telegram.me/'..member..'\n<b>»Status: User Globally UnBanned!« </b>\n')
...
local chat_id = msg.to.id
local hash = 'gbanned'
send_large_msg(get_receiver(msg), "\n<b>Globall Ban List  Has Been Cleaned </b>")
redis:del(hash)
end
  if matches[1]:lower() == 'ban' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
        ...
        if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
          	return "\n<b>you Can't Ban Mods/Owner/Admins"
        end
        if tonumber(matches[2]) == tonumber(msg.from.id) then
          	return "\n<b>You Can't Ban Your Self !"
        end
        local print_name = user_print_name(msg.from):gsub("‮", "")
	    local name = print_name:gsub("_", "")
		local receiver = get_receiver(msg)
...
local bannedhash = 'banned:'..msg.from.id..':'..msg.to.id
        ...
        local bannedhash = 'banned:'..msg.from.id..':'..msg.to.id
        local banned = redis:get(bannedhash)
	send_large_msg(receiver, '\n<b>User </b><code>'..matches[2]..' </code><b>banned!</b>')
local bannedhash = 'banned:'..msg.from.id..':'..msg.to.id
        redis:incr(bannedhash)
        local bannedhash = 'banned:'..msg.from.id..':'..msg.to.id
        local banned = redis:get(bannedhash)
  ...
  if matches[1]:lower() == 'unban' then -- /unban
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      ...
      if string.match(targetuser, '^%d+$') then
        	...
        	return '\n<b>User </b><code>'..user_id..' </code><b>unbanned! </b>'
      else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unban',
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return '\n<b>User </b><code>'..user_id..' </code><b>Globally Banned! </b>'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'banall',
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return '\n<b>User </b><code>'..user_id..' </code><b>Globally UnBanned! </b>'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unbanall',
