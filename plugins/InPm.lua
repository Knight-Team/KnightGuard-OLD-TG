...
local service = msg.service
	if to == 'user' and msg.fwd_from then
		...
		text = "User From Info:\n#-#-#-#-#-#-#-#-#-#-#-#-#-#-#\nID: "..from_id.."\nFirst: "..from_first_name.."\nLast: "..from_last_name.."\nUsername: @"..from_username.."\nHis Link:\nhttps://telegram.me/"..from_username.."\n#-#-#-#-#-#-#-#-#-#-#-#-#-#-#"
...
if to == 'user' or service or is_admin1(msg) and to == "chat" or to == "channel" then
	...
	local data = load_data(_config.moderation.data)
	if matches[2]:lower() == 'owner' and matches[3]:lower() == 'support' then
		savelog(msg.to.id, name_log.." ["..msg.from.id.."] tried to join English support")
		local target = 1077666255
	...
	elseif matches[2]:lower() == 'knightguard' and matches[3]:lower() == 'support' then
		savelog(msg.to.id, name_log.." ["..msg.from.id.."] tried to join Persian support")
		local target = 1054786249
		local long_id = data[tostring(target)]['long_id']
		if is_banned(msg.from.id, tostring(target)) then
			return 'You Are Banned.'
		end
	...
	if matches[1] == 'help' and msg.to.type == 'user' or matches[1] == 'pmhelp' and is_admin1(msg) and msg.to.type ~= 'user' then
		...
		text = "#-#-#-#-#-#-#-#-#-#-#-#-#-#\nHi Welcom TO Knight Guard\nFor See SuperGroup Help Commands Send:\n/superhelp\nIf Need Help For Mange Your Group Send:\nFeedback *\n* Is Your Text\nFor Buy Bot Send Feedback Command\nChannel:@Knight_Team\n#-#-#-#-#-#-#-#-#-#-#-#-#-#"
     	return text
    end
